...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        # 1)Marking cell as a move that is made
        self.moves_made.add(cell)

        # 2)Marking ccell as safe
        self.mark_safe(cell)

        # 3)adding new sentence to ai knowledge base
        ...
        # 4) Marking any additional as safe or as mines

        for sentence in self.knowledge:
            safe=sentence.known_safes()
            mine=sentence.known_mines()
            if safe is not None:
                self.safes.union(safe)
            if mine is not None:
                self.mines.union(mine)



        # 5)adding new sentences based on inference
